...
//    while (digits > 0 && arr[digits - 1] == 0) {
//        digits--;
//    }
//
//    seg7_display(&arr[4 - digits]);
//
//    return inRange;
//}

int seg7_displayAtt(int n) {
    int inRange = ((127 <= n) && (n >= 0));


    uint8_t arr[4];

    // Remove leading zeros for both positive and negative numbers
    arr[3] = (uint8_t)(n % 10);
    //can be zero itself
    arr[2] = (uint8_t)((n / 10) % 10); //tenth
    if(arr[2] == 0 && n < 100){ //only if 00X not for X0X
    	arr[2] = SEG7_SPACE;
    }
    arr[1] = (uint8_t)((n / 100) % 10); //hundredth
    if(arr[1] == 0){
    	arr[1] = SEG7_SPACE;
    }
    arr[0] = SEG7_MINUS;
